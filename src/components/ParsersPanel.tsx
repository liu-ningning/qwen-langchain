...
import {
  ...
  Btn, FieldLabel, Select, Textarea, Tag,
  PanelHeader, SplitLayout, Col, Sep, Spinner,
} from './ui'

type ParserType = 'string' | 'json' | 'list'

const PARSER_INFO: Record<ParserType, { label: string; desc: string; color: string }> = {
  string: { label: 'StringOutputParser',            desc: '直接提取 .content 文本，最常用',          color: 'var(--accent)' },
  json:   { label: 'JsonOutputParser',              desc: '解析 JSON 字符串为 JS 对象',              color: 'var(--green)' },
  list:   { label: 'CommaSeparatedListOutputParser', desc: '解析逗号分隔的列表为数组',               color: 'var(--amber)' },
}

function buildPrompt(type: ParserType, input: string): string {
  if (type === 'string') return `请分析以下产品评价，给出简洁的总体评价：\n\n${input}`
  if (type === 'json')   return `分析以下内容，只返回 JSON（不要 markdown 代码块），格式：{"sentiment":"positive/negative/neutral","score":0.0,"keywords":["词1","词2"],"summary":"一句话总结"}\n\n${input}`
  return `从以下内容中提取 5 个关键词，只返回逗号分隔的词语，不要其他任何文字：\n\n${input}`
}

function parseOutput(type: ParserType, raw: string): { result: string; error?: string } {
  ...
  if (type === 'json') {
    try {
      ...
      return { result: raw, error: `JSON.parse 失败: ${e instanceof Error ? e.message : String(e)}` }
    }
  }
  // list
...
export default function ParsersPanel({ apiKey, model }: { apiKey: string; model: string }) {
  const [type, setType] = useState<ParserType>('string')
  const [input, setInput] = useState('「这款耳机音质真的太棒了，佩戴也很舒适，就是价格稍贵了一点，总体来说非常值得购买！」')
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
      <PanelHeader title="Output Parsers · 输出解析器" desc="StringOutputParser 提取纯文本；JsonOutputParser 解析 JSON；CommaSeparated 解析列表。">
      ...
      <SplitLayout
        ...
        left={
          <Card>
            <CardHead><CardTitle>解析器配置</CardTitle></CardHead>
            <CardBody>
              <Col>
                <div>
                ...
                <div style={{
                  ...
                  <div style={{ color: info.color, fontFamily: 'var(--mono)', fontWeight: 600, marginBottom: 4, fontSize: 11 }}>
                    {info.label}
                  </div>
                ...
                <div style={{
                  padding: '9px 11px', borderRadius: 6, fontSize: 12,
        ...
        right={
          <Card style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
            <CardHead><CardTitle>原始输出 → 解析结果</CardTitle></CardHead>
            <CardBody style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div>
                ...
                <OutputBox style={{ minHeight: 80 }}>
                  {loading ? <Spinner /> : rawOutput || <span style={{ color: 'var(--text3)' }}>-</span>}
                </OutputBox>
              </div>
