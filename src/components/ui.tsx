...
export function CardTitle({ children }: { children: ReactNode }) {
  return <span style={{ fontFamily: 'var(--mono)', fontSize: 11, color: 'var(--text2)', fontWeight: 600 }}>{children}</span>
}
...
export function OutputBox({
  children, style, minHeight = 100,
}: {
  children?: ReactNode; style?: CSSProperties; minHeight?: number
}) {
  return (
    <div style={{
      ...
      overflowY: 'auto', minHeight, ...style,
    }}>
      {children}
    </div>
  )
...
const btnStyles: Record<BtnVariant, CSSProperties> = {
  ...
  danger:    { background: 'var(--red-dim)', color: 'var(--red)', border: '1px solid rgba(244,63,94,.25)' },
  green:     { background: 'var(--green-dim)', color: 'var(--green)', border: '1px solid rgba(16,217,160,.25)' },
}
export function Btn({
  children, onClick, disabled, variant = 'primary', style, title,
}: {
  children: ReactNode; onClick?: () => void; disabled?: boolean
  variant?: BtnVariant; style?: CSSProperties; title?: string
}) {
  return (
    <button
      onClick={onClick}
...
export function FieldLabel({ children }: { children: ReactNode }) {
  return <div style={{ fontSize: 11, color: 'var(--text3)', fontFamily: 'var(--mono)', marginBottom: 4 }}>{children}</div>
}
...
export function Input({
  ...
  type?: string; style?: CSSProperties; onKeyDown?: React.KeyboardEventHandler<HTMLInputElement>
}) {
  return (
    <input
      type={type}
...
export function Tag({ children, color = 'blue' }: { children: ReactNode; color?: TagColor }) {
  return (
    <span style={{
      ...
      fontSize: 11, fontFamily: 'var(--mono)', marginRight: 6,
      ...tagColors[color],
    }}>
      {children}
    </span>
...
export function PanelHeader({
  ...
  return (
    <div style={{
      ...
      <div style={{ fontFamily: 'var(--sans)', fontSize: 19, fontWeight: 700, marginBottom: 4 }}>
        {title}
      </div>
      {desc && (
...
export function SplitLayout({ left, right, leftWidth = 320 }: {
  left: ReactNode; right: ReactNode; leftWidth?: number
}) {
  return (
    <div style={{ display: 'flex', flex: 1, gap: 16, padding: '18px 20px', overflow: 'hidden', minHeight: 0 }}>
      <div style={{ width: leftWidth, minWidth: leftWidth, display: 'flex', flexDirection: 'column', gap: 12 }}>
        {left}
      </div>
...
// ── Flex helpers ──────────────────────────────────────────────
export function Row({ children, gap = 8, style }: { children: ReactNode; gap?: number; style?: CSSProperties }) {
  return <div style={{ display: 'flex', gap, alignItems: 'flex-end', flexWrap: 'wrap', ...style }}>{children}</div>
}

export function Col({ children, gap = 10, style }: { children: ReactNode; gap?: number; style?: CSSProperties }) {
