...
import {
  ...
  Btn, FieldLabel, Input, Select, Tag,
  PanelHeader, SplitLayout, Col, Row, Sep, Spinner,
} from './ui'

export default function RAGPanel({ apiKey, model }: { apiKey: string; model: string }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
      <PanelHeader title="RAG · 知识库检索" desc="模拟向量检索：相似度搜索 → 拼入 Prompt → LLM 回答。对比有/无 RAG 的效果差异。">
      ...
      <SplitLayout
        ...
        left={
          <>
            <Card>
              <CardHead><CardTitle>知识库文档（{KB_DOCS.length} 条）</CardTitle></CardHead>
              <CardBody style={{ maxHeight: 280, overflowY: 'auto', padding: '10px 12px' }}>
                <Col gap={6}>
                  {KB_DOCS.map(doc => {
                    ...
                    return (
                      <div key={doc.id} style={{
                        padding: '9px 11px', borderRadius: 7, fontSize: 12, lineHeight: 1.5,
                        ...
                        {score !== undefined && (
                          <div style={{ height: 2, borderRadius: 1, background: 'var(--border)', marginTop: 5, overflow: 'hidden' }}>
                            <div style={{ height: '100%', width: `${score * 100}%`, background: 'var(--green)', transition: 'width .6s' }} />
                          </div>
                        )}
                      </div>
                    )
            ...
            <Card>
              <CardHead><CardTitle>查询配置</CardTitle></CardHead>
              <CardBody>
                <Col>
                  <div>
                  ...
                  <Row gap={8}>
                    <Btn onClick={() => run(true)} disabled={loading} style={{ flex: 1 }}>
                      {loading ? <Spinner /> : '▶'} 有 RAG
                    </Btn>
                    <Btn onClick={() => run(false)} disabled={loading} variant="secondary" style={{ flex: 1 }}>
                      无 RAG 对比
                    </Btn>
                  </Row>
                </Col>
        ...
        right={
          <Col gap={12} style={{ flex: 1, overflow: 'hidden' }}>
            {retrieved.length > 0 && (
              <Card>
                <CardHead><CardTitle>检索到的文档</CardTitle></CardHead>
                <CardBody>
                  <Col gap={8}>
                    {retrieved.map(doc => (
                      <div key={doc.id} style={{
                        ...
                        <div style={{ fontSize: 10, color: 'var(--green)', marginBottom: 4, fontFamily: 'var(--mono)' }}>
                          相似度 {((doc.score ?? 0) * 100).toFixed(0)}% · {doc.source}
                        </div>
                        {doc.text}
                      </div>
                    ))}
            ...
            <Card style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
              ...
              <CardBody style={{ flex: 1 }}>
                <OutputBox style={{ minHeight: 120 }}>
                  {loading ? <Spinner /> : answer || <span style={{ color: 'var(--text3)' }}>等待执行...</span>}
                </OutputBox>
              </CardBody>
            </Card>
          </Col>
